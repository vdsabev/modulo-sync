...
config_1.config.events.map((configEvent) => {
    ...
    utils_1.keys(configEvent).map((definition) => {
        ...
        settings.push(Object.assign({}, config_1.parseEventDefinition(definition), { path }));
    });
    ...
    events.map((event) => {
        ...
        const sourcePattern = pattern_1.pattern(event.path);
        actions.map((action) => {
            const { plugin: destination } = require(`./plugins/${action.plugin}`);
            const destinationPattern = pattern_1.pattern(action.path);
